...
const getFeedPosts = async (userId, limit = 20, offset = 0) => {
  const result = await query(
    ...
    FROM posts p
    JOIN users u ON p.user_id = u.id
    WHERE p.user_id IN (SELECT following_id FROM follows WHERE user_id = $1)
    ORDER BY p.created_at DESC
    LIMIT $2 OFFSET $3`,
    [userId, limit, offset],
  );
...
const updatePost = async (postId, userId, updatedData) => {
  const result = await query(
    "UPDATE posts SET content = $2, media_url = $3 WHERE id = $1 AND user_id = $4",
    [postId, updatedData.content, updatedData.media_url, userId],
  );
...
const searchPostsByContent = async (search, limit = 20, offset = 0) => {
  const result = await query(
    `SELECT p.*, u.username, u.full_name
     FROM posts p
     JOIN users u ON p.user_id = u.id
